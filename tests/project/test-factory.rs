...
mod tests {
    ...
    use library_blockchain::transaction::*;    
    ...
    #[test]
    fn blockchain_factory<'a>()
    {  
        let difficulty:u128= 0x00ffffffffffffffffffffffffffffff;
        let mut blockchain:Blockchain<'a>=Blockchain::new();

        let json_value=sample_trx_json_data_from_module().unwrap();

        let mut maked_transactions_of_a_block:Vec<Transaction<'a>>=vec![];
        let serde_values_transactions:serde_json::Value= serde_json::from_value(json_value).unwrap(); 
        let blocks_val:serde_json::Value=serde_values_transactions["blocks"].clone();  
                ...
                //let refcell_trx=rc_maked_transactions_of_a_block;                        
                    let mut c:Cell<&[Transaction<'a>]>=Cell::new(&maked_transactions_of_a_block.as_slice());
